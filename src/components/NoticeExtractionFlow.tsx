...
export default function NoticeExtractionFlow({ onProceedToDraft }: Props) {
  ...
  const [extractedData, setExtractedData] = useState<ExtractedNotice | null>(null);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("extract-notice-fields", {
        body: { pdfBase64 },
      });

      if (error) {
        throw new Error(error.message || "Failed to parse edge function response.");
      }

      if (!data?.success || !data?.data) {
        throw new Error(data?.error || "AI failed to extract fields from this document.");
      }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="mb-6 border-b pb-4">
        <h1 className="text-xl font-bold text-gray-900">Step A: Notice Parsing</h1>
        <p className="text-sm text-gray-500">
          Upload a GST/Income Tax Notice PDF to extract structured data for drafting.
        </p>
      </div>
      ...
      {step === "EXTRACTING" && (
        <div className="py-12 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-base font-semibold text-gray-800">Reading and parsing document...</p>
          <p className="text-xs text-gray-400 mt-1">Extracting dates, references, and discrepancy details from {fileName}</p>
        </div>
      )}
        ...
        <form onSubmit={handleFinalSubmit} className="space-y-5">
          ...
          {extractedData.confidence_notes && (
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <p className="text-sm text-amber-900 mt-0.5">{extractedData.confidence_notes}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Notice Type */}
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Notice Type</label>
              <input
                type="text"
                value={extractedData.notice_type || ""}
                onChange={(e) => handleFieldChange("notice_type", e.target.value)}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleFieldChange("notice_ref_no", e.target.value)}
                className="w-full px-3 py-2 border rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleFieldChange("notice_date", e.target.value)}
                className="w-full px-3 py-2 border rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none"
              />
            </div>
            ...
            {/* Financial Year */}
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Financial Year (FY)</label>
              <input
                type="text"
            ...
            {/* Period From & To */}
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Period From</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Period To</label>
              <input
                type="date"
            ...
            {/* GSTIN */}
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">GSTIN Mentioned</label>
              <input
                type="text"
                value={extractedData.gstin_mentioned || ""}
                onChange={(e) => handleFieldChange("gstin_mentioned", e.target.value)}
                className="w-full px-3 py-2 border rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none"
              />
            </div>
            ...
            {/* Ward / Circle / Range */}
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Ward / Circle / Range</label>
              <input
                type="text"
                value={extractedData.ward_circle_range || ""}
                onChange={(e) => handleFieldChange("ward_circle_range", e.target.value)}
                className="w-full px-3 py-2 border rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none"
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">Issuing Office Name</label>
            <input
              type="text"
              value={extractedData.issuing_office_name || ""}
              onChange={(e) => handleFieldChange("issuing_office_name", e.target.value)}
              className="w-full px-3 py-2 border rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none"
            />
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">Discrepancy Summary</label>
            <input
              type="text"
              value={extractedData.discrepancy_type || ""}
              onChange={(e) => handleFieldChange("discrepancy_type", e.target.value)}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">Full Discrepancy Details</label>
            <textarea
              rows={3}
              value={extractedData.discrepancy_details || ""}
              onChange={(e) => handleFieldChange("discrepancy_details", e.target.value)}
              className="w-full px-3 py-2 border rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none"
            />
          </div>
